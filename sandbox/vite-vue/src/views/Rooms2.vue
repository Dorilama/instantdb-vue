<template>
  <div class="flex flex-col items-center pt-4 pb-8 px-2 gap-4">
    ...
    <div class="card card-bordered rounded-lg bg-base-100 sm:min-w-96">
      <div class="card-body">
        <div class="form-control">
          <label v-for="r of allRooms" :key="r" class="label cursor-pointer">
            ...
            <input type="radio" name="radio-room" class="radio" :value="r" v-model="roomId" />
          </label>
        </div>
      </div>
      <div class="overflow-x-auto w-full">
        <table class="table rounded-t-none rounded-b-lg" :class="presence.isLoading.value ? 'skeleton' : ''">
          <!-- head -->
          <thead>
            <tr>
              <th></th>
      ...
      <div v-if="presence.error?.value" ref="alert-error" role="alert"
